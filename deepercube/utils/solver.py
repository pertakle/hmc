...
def solve_beam(goal: ko.Kostka, agent: A2CCubeAgent, beam_size: int, limit: int) -> int:
    ...
    for step in range(limit):
        ...
        nej_indexy = np.argsort(pr_nasledniku_vektor)[
            -beam_size:
        ]  # argsort setridi vzestupne
        ...
        assert len(pr_kandidatu) == len(
            kandidati
        ), f"Pocet kandidatu ({len(kandidati)}) a pravdepodobnosti ({len(pr_kandidatu)}) musi byt stejny."
        assert (
            kandidati.shape[1:] == goal.shape
        ), f"Nespravny tvar kandidatu {kandidati.shape}."
        assert (
            len(kandidati) <= beam_size
        ), f"Prilis mnoho kandidatu {len(kandidati)}, limit je {kandidatu}."

    return 1 if np.any(kv.je_stejna(kandidati, goal)) else 0


def solve_beam_vek(
    goal: kv.KostkaVek, agent: A2CCubeAgent, beam_size: int, limit: int
) -> int:
    solved = 0
    for g in goal:
...
def any_solved(
    states: KostkaVekBatch, goals: kv.KostkaVek
) -> np.ndarray[Tuple[B], np.dtype[np.bool_]]:
    solved = np.full(len(states), False)
    return solved
    ...


def solve_beam_vekevk(
    goals: kv.KostkaVek, agent: A2CCubeAgent, beam_size: int, limit: int
) -> int:
...
def solve_greedy_vek(
    cilova_kostka: kv.KostkaVek, agent: A2CCubeAgent, limit: int
) -> int:
    akt_kostka = kv.nova_kostka_vek(len(cilova_kostka))
    slozene = kv.je_stejna(akt_kostka, cilova_kostka)
    for _ in range(limit):
        predikce = agent.predict_action_probs(
            agent.merge_states_and_goals(akt_kostka, cilova_kostka)
        )
